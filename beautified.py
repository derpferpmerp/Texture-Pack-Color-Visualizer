...
def rcolored(txt):
    ...
    argb = list([
        list(map(str.strip,
                 g.strip('][').replace('"', '').split(',')))
        for g in list(([x[1] for x in lstvals]))
    ])
    ...
    for x in range(len(lstkeysinfo)):
        if x != len(lstkeysinfo) - 1:
            finalstring += prrgb(
                lstvalsinfo[x],
                str("".join(enumerated[int(lstkeysinfo[int(x)]
                                           ):int(lstkeysinfo[int(x) + 1]) -
                                       2])))
        else:
            finalstring += prrgb(
                lstvalsinfo[x],
                "".join(enumerated[int(lstkeysinfo[int(x)]):len(txt)]))
    if finalstring != None:
        print(f"{finalstring}")
...
lstodesc = list(
    ["\n\t" + str(g).replace(u"\u00A7", "?") + "\n" for g in lstdesc])
lstnames = list([
    f"?fUsing?f ?b{list([k for k in finallst[o].keys()])[0]}?b?f.?f?amcmeta?a?f:?f\n{lstodesc[o]}"
    for o in range(len(finallst))
])
print("\n")
[
    f"{rcolored(lstnames[x])}" for x in range(len(lstnames))
